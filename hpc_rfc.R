...
#install.packages("randomForest",repos="https://mirrors.tuna.tsinghua.edu.cn/CRAN/",lib="/home/julius_bs/xchen/R-4.2.2/library")
...
#rfc
s=Sys.time()
data_rfc<-data.frame(o3,s3,brier,sd3,o4,s4,accuracy,sd4)
rep<-1000
train_rfc<-array(0,c(length(grad),rep,2))
test_rfc<-array(0,c(length(grad),rep,2))

for (i in 1:length(grad)){
  
  #for (j in 1:rep/mul){
  rfclist<-foreach(k = 1:rep) %dopar%{
    trainset<-datagenerator(grad[i],coeff,-1.08)[,c(1:23,26)]
    rfc <- randomForest(factor(event) ~ ., data = trainset, importance = TRUE)
    
    #applied in itself
    trainset.rfc.predict<-as.numeric(predict(rfc,trainset,type="class"))-1
    #mul*(j-1)+k    
    tmp1<-c(mean((trainset[,24]-trainset.rfc.predict)^2),mean(1-(trainset[,24]-trainset.rfc.predict)^2))
    
    #applied in testset   
    test.rfc.predict<-as.numeric(predict(rfc,test,type="class"))-1
    tmp2<-c(mean((test[,26]-test.rfc.predict)^2),mean(1-(test[,26]-test.rfc.predict)^2))
    
    return(list(tmp1,tmp2))
  }#k=rep
  for(t in 1:rep){
    train_rfc[i,t,]<- rfclist[[t]][[1]]
    test_rfc[i,t,]<- rfclist[[t]][[2]]
  }
  #  }#j=rep/mul
  
  data_rfc[i,1]<-  mean(train_rfc[i,,1]-test_rfc[i,,1])
  data_rfc[i,2]<- sd(train_rfc[i,,1]-test_rfc[i,,1])
  data_rfc[i,3]<- mean(test_rfc[i,,1])
  data_rfc[i,4]<- sd(test_rfc[i,,1])
  
  data_rfc[i,5]<- mean(train_rfc[i,,2]-test_rfc[i,,2])
  data_rfc[i,6]<- sd(train_rfc[i,,2]-test_rfc[i,,2])
  data_rfc[i,7]<- mean(test_rfc[i,,2])
  data_rfc[i,8]<- sd(test_rfc[i,,2])
...
e=Sys.time()
print(e-s)
write.table(data_rfc,"rfc.csv",row.names=FALSE,col.names=TRUE,sep=",")
write.table(train_rfc,"train_rfc.csv",row.names=FALSE,col.names=TRUE,sep=",")
write.table(test_rfc,"test_rfc.csv",row.names=FALSE,col.names=TRUE,sep=",")